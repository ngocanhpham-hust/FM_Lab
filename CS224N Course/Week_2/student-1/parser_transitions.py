...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        # Buffer contains all words in the sentence
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":  # SHIFT
            ...
            # Add dependency (head = stack[-1], dependent = stack[-2])
            head = self.stack[-1]
            dependent = self.stack[-2]
            self.dependencies.append((head, dependent))
            # Remove dependent (stack[-2]) from stack
            del self.stack[-2]

        elif transition == "RA":  # RIGHT-ARC
            # Add dependency (head = stack[-2], dependent = stack[-1])
            head = self.stack[-2]
            dependent = self.stack[-1]
            self.dependencies.append((head, dependent))
            # Remove dependent (stack[-1]) from stack
            self.stack.pop()
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # 1. Initialize partial parses for all sentences
    partial_parses = [PartialParse(sentence) for sentence in sentences]
    unfinished_parses = partial_parses[:]

    # 2. Loop until all parses are finished
    while len(unfinished_parses) > 0:
        # Take next minibatch
        minibatch = unfinished_parses[:batch_size]

        # Predict next transition for each parse in batch
        ...
        # Remove completed parses (stack has only ROOT and buffer is empty)
