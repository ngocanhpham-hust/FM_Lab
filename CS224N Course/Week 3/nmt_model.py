...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # Conv1d expects (batch, channels, seq_len). We will use padding='same' to preserve length.
        # padding='same' requires recent PyTorch; if unavailable, alternative padding calculation may be needed.
        self.post_embed_cnn = nn.Conv1d(in_channels=embed_size,
                                        out_channels=embed_size,
                                        kernel_size=2,
                                        padding='same')

        # Bidirectional single-layer LSTM encoder (input size = embed_size, hidden size = hidden_size)
        self.encoder = nn.LSTM(input_size=embed_size,
                               hidden_size=hidden_size,
                               bidirectional=True)

        # Decoder is an LSTMCell which receives [y_t_embed ; o_{t-1}] of size (embed_size + hidden_size)
        self.decoder = nn.LSTMCell(input_size=embed_size + hidden_size,
                                   hidden_size=hidden_size)

        # Projections to initialize decoder hidden and cell from encoder final states (2*h -> h)
        self.h_projection = nn.Linear(2 * hidden_size, hidden_size, bias=False)
        self.c_projection = nn.Linear(2 * hidden_size, hidden_size, bias=False)

        # Attention projection: project encoder hiddens (2h) into h
        self.att_projection = nn.Linear(2 * hidden_size, hidden_size, bias=False)

        # Combined output projection: W_u: maps [h_dec; a_t] (h + 2h = 3h) to h
        self.combined_output_projection = nn.Linear(3 * hidden_size, hidden_size, bias=False)

        # Final projection to vocabulary: from hidden h to vocab size
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        # 1) Embedding lookup: source_padded has shape (src_len, b)
        X = self.model_embeddings.source(source_padded)  # (src_len, b, e)

        # 2) Apply CNN: permute to (b, e, src_len), apply conv1d, then permute back to (src_len, b, e)
        ...
        # 3) Pack, run through encoder, and unpack
        ...
        # Convert enc_hiddens to shape (b, src_len, 2h)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)  # (b, src_len, 2h)

        # 4) Prepare initial decoder state by concatenating forward and backward final states
        # last_hidden, last_cell: shape (num_layers * num_directions, b, h) = (2, b, h)
        # Concatenate forward and backward states for hidden and cell
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 1) Project encoder hidden states for attention (b, src_len, h)
        enc_hiddens_proj = self.att_projection(enc_hiddens)  # (b, src_len, h)

        # 2) Target embeddings: target_padded is (tgt_len, b)
        Y = self.model_embeddings.target(target_padded)  # (tgt_len, b, e)

        # 3) Iterate over time steps
        for Y_t in torch.split(Y, 1, dim=0):
            Y_t = Y_t.squeeze(0)  # (b, e)
            # Concatenate Y_t and o_prev to form Ybar_t
            Ybar_t = torch.cat([Y_t, o_prev], dim=1)  # (b, e + h)
            # Step: update decoder state and get new combined output
            dec_state, o_t, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(o_t)
            o_prev = o_t

        # 4) Stack combined outputs -> (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 1) Apply LSTMCell: returns (h_t, c_t)
        dec_hidden, dec_cell = self.decoder(Ybar_t, dec_state)
        dec_state = (dec_hidden, dec_cell)

        # 2) Compute attention scores: enc_hiddens_proj (b, src_len, h), dec_hidden (b, h)
        # Use bmm: need (b, src_len, h) @ (b, h, 1) -> (b, src_len, 1), then squeeze -> (b, src_len)
        ...
        # 1) Softmax over source positions
        alpha_t = F.softmax(e_t, dim=1)  # (b, src_len)

        # 2) Compute attention-weighted sum of enc_hiddens -> a_t (b, 2h)
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)  # (b, 2h)

        # 3) Concatenate dec_hidden (b,h) and a_t (b,2h) -> U_t (b, 3h)
        U_t = torch.cat((dec_hidden, a_t), dim=1)  # (b, 3h)

        # 4) Apply combined output projection: V_t (b, h)
        V_t = self.combined_output_projection(U_t)

        # 5) Apply tanh and dropout -> O_t
